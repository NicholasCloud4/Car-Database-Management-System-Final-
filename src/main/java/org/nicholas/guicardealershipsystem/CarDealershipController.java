...
public class CarDealershipController {
     ...
     */
    private Connection getConnection(String filePath) throws SQLException {
     ...
     * @param filePath sets the file path
     */
    public void loadCarsFile(String filePath) {
        // Connecting to the database based on the file that the user has uploaded
        try (Connection connection = getConnection(filePath); PreparedStatement statement = connection.prepareStatement("SELECT * FROM Cars"); ResultSet resultSet = statement.executeQuery()) {
    ...
    /**
     * Name: FXML removeSelectedCar
     * <p>
     * Removes the selected car from the view and database.
     */
    @FXML
    private void removeSelectedCar() {
        if (filePath != null && !filePath.isEmpty()) {
            removeSelectedCar(filePath);
        } else {
     ...
     * @param filePath sets the file path
     */
    private void removeSelectedCar(String filePath) {
        Car selectedCar = tableView.getSelectionModel().getSelectedItem();
        if (selectedCar != null) {
            try (Connection connection = getConnection(filePath);
     ...
     *
     * @param selectedCar gets selected car to be edited
     * @param filePath    sets the file path
     */
    @FXML
    private void editCarDialog(Car selectedCar, String filePath) {
        ...
        engineField.setPromptText("Engine");
        ...
        dialog.setResultConverter(buttonType -> {
            if (buttonType == saveButton) {
                ...
                try (Connection connection = getConnection(filePath); PreparedStatement statement = connection.prepareStatement("UPDATE Cars SET year=?, make=?, model=?, color=?, engine=?, transmission=?, price=?, sold=? WHERE id=?")) {
    ...
    private void uploadFile() {
        ...
        if (selectedFile != null) {
            String filePath = selectedFile.getAbsolutePath(); //This is to get the File PATH form user
    ...
    /**
     * Name: FXML addCarDialog1
     * <p>
     * Add car to the view and database.
     * It calls addCarDialog(String filePath)
     */
    @FXML
    private void addCarDialog() {
        if (filePath != null && !filePath.isEmpty()) {
            addCarDialog(filePath);
        } else {
            System.err.println("Error: File path is not set.");
        }
    }
     ...
     * @param filePath sets the file path
     */
    private void addCarDialog(String filePath) {
        Dialog<Car> dialog = new Dialog<>();
        ...
        dialog.setResultConverter(buttonType -> {
            if (buttonType == addButton) {
                ...
                try (Connection connection = getConnection(filePath); PreparedStatement statement = connection.prepareStatement("INSERT INTO Cars (year, make, model, color, engine, transmission, price, sold) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    /**
     * Name: FXML removeFilteredCars
     * <p>
     * Removes the filtered cars that the users types
     * It calls removeFilteredCars(String filePath)
     */
    @FXML
    private void removeFilteredCars() {
        if (filePath != null && !filePath.isEmpty()) {
            removeFilteredCars(filePath);
        } else {
            System.err.println("Error: File path is not set.");
        }
    }
     ...
     * @param filePath sets the file path
     */
    private void removeFilteredCars(String filePath) {
